import { Model, Document, QueryOptions, UpdateQuery, PopulateOptions } from 'mongoose';
...
export abstract class BaseRepository<T extends Document> {
    ...
    async find(
        ...
        let query = this.model.find({ isDeleted: { $ne: true }, ...filter }, null, options);
        if (populate) {
    ...
    async count(filter: any = {}): Promise<number> {
        return this.model.countDocuments({ isDeleted: { $ne: true }, ...filter }).exec();
    }
}
