...
export class ScraperService {
    ...
    async scrapeCategories() {
        ...
        try {
            ...
            await page.goto('https://cellphones.com.vn/', { waitUntil: 'networkidle2', timeout: 60000 });
                ...
                items.forEach(item => {
                    ...
                    links.forEach(link => {
                        ...
                        if (!url.startsWith('http')) {
                            url = 'https://cellphones.com.vn' + (url.startsWith('/') ? '' : '/') + url;
                        }
    ...
    async scrapeProducts(categoryUrl: string = 'https://cellphones.com.vn/mobile.html', limit: number = 5) {
        ...
        try {
            ...
            const productLinks = await page.evaluate(() => {
                ...
                return links.map(link => link.getAttribute('href')).filter(href => href && href.includes('.html'));
            });

            this.logger.log(`Found ${productLinks.length} products. Limiting to ${limit}.`);
            ...
            for (const link of targetLinks) {
                ...
                try {
                    ...
                    if (productData) {
                        ...
                        const category = categoryUrl.split('/').pop()?.replace('.html', '') || 'unknown';
    ...
    private async scrapeProductDetail(browser: puppeteer.Browser, url: string) {
        ...
        try {
            ...
            const data = await page.evaluate(() => {
                ...
                const priceStr = getText(['.product__price--show', '.tpt---price', '.box-info__price', 'p.special-price']);

                // Improved Image extraction (handling placeholders/lazy-loading)
                const imageElements = Array.from(document.querySelectorAll('.swiper-slide img, .gallery-image img, #product-image-main'));
                const images = imageElements
                    .map(img => {
                        const src = img.getAttribute('data-src') || img.getAttribute('src') || '';
                        // Filter out small placeholders or gifs
                        if (src.includes('placeholder') || src.endsWith('.gif')) return null;
                        return src;
                    })
                ...
                const technicalItems = document.querySelectorAll('.technical-content-item, .box-technical-info tr');
                technicalItems.forEach(item => {
                    const label = item.querySelector('p:first-child, th')?.textContent?.trim();
